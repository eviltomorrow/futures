...
func TestLogFileNoPermission(t *testing.T) {
	...
	require.NoError(t, os.Mkdir(readOnlyDirPath, 0o755))
	require.NoError(t, os.Chmod(readOnlyDirPath, 0o555)) // Read-only directory
	conf = &Config{
		Level: "info",
		File: FileLogConfig{
	...
	_, _, err = InitLogger(conf)
	require.Contains(t, err.Error(), "permission denied")
	require.NoError(t, os.Chmod(readOnlyDirPath, 0o755))

	// Using a directory as log file
	dirLogPath := filepath.Join(tempDir, "dir-as-log")
	require.NoError(t, os.Mkdir(dirLogPath, 0o755))
	conf = &Config{
		Level: "info",
		File: FileLogConfig{
			Filename: dirLogPath,
