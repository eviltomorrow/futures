...
root_dir=$(pwd)
PATH=${root_dir}/third-party/protoc-gen-go/${GOOS}_${GOARCH}:$PATH
...
for name in $(ls ${root_dir}/lib/entity); do
    ...
    if [ "${name##*.}"x = "proto"x ]; then
        ...
        ${root_dir}/third-party/protoc/${GOOS}_${GOARCH}/bin/protoc --proto_path="${root_dir}" --proto_path="${root_dir}/lib/entity" --go_opt=paths=source_relative --go_out="${root_dir}/lib/grpc/pb/entity" --go-grpc_opt=paths=source_relative --go-grpc_out="${root_dir}/lib/grpc/pb/entity" ${name}
        code=$(echo $?)
        if [ $code = 0 ]; then
    ...
    if [ -d ${adapter_dir} ]; then
        ...
        if [ ${ok}=true ]; then
            ...
            for name in $(ls ${adapter_dir}); do
                ...
                ${root_dir}/third-party/protoc/${GOOS}_${GOARCH}/bin/protoc --proto_path="${root_dir}" --proto_path="${adapter_dir}" --go_opt=paths=source_relative --go_out="${pb_dir}" --go-grpc_opt=paths=source_relative --go-grpc_out="${pb_dir}" ${name}
                code=$(echo $?)
                if [ $code = 0 ]; then
                    echo -e "编译文件: ${file_name} => [\033[32m成功\033[0m] "
                else
